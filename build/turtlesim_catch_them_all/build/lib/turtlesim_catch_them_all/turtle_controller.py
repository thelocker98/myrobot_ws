...
class TurtleControllerNode(Node):
    def __init__(self):
        ...
        self.get_logger().info("Start Turtle Sim Controller")
    ...
    def callback_alive_turtles(self, msg):
        if len(msg.turtles) > 0:
            if self.catch_closest_turtle_first_:
                closest_turtle = None
                clostest_turtle_distance = None

                for turtle in msg.turtles:
                    dist_x = turtle.x - self.pose_.x
                    dist_y  = turtle.y - self.pose_.y
                    distance = math.sqrt(dist_x**2 + dist_y**2)
                    if closest_turtle == None or distance < clostest_turtle_distance:
                        closest_turtle = turtle
                        clostest_turtle_distance = distance
    ...
    def callback_catch_turtle(self, future, turtle_name):
        try:
            ...
            if response.success:
                ...
                self.get_logger().error("Turtle " + turtle_name + " had an error")
